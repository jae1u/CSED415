...
def handle_https(client_conn):
    ...
    try:
        ...
        try:
            ...
            tls_server_conn = ssl.create_default_context().wrap_socket(raw_sock, server_hostname=host)
        ...
        def forward(src, dst, direction, shutdown_event):
            try:
                ...
                print(f"[HTTPS {direction}] Forward error: {e}")
            finally:
                shutdown_event.set()

        t1 = threading.Thread(target=forward, args=(tls_client_conn, tls_server_conn, "C→S", shutdown_event))
        t2 = threading.Thread(target=forward, args=(tls_server_conn, tls_client_conn, "S→C", shutdown_event))
...
def handle_http(client_conn):
    ...
    try:
        ...
        def forward_http(src, dst, direction, shutdown_event):
            try:
                while not shutdown_event.is_set():
                    try:
                        data = src.recv(4096)
                        if not data:
                            break
                        print(f"[HTTP {direction}] {len(data)} bytes")
                        dst.sendall(data)
                    except (ConnectionResetError, BrokenPipeError, OSError):
                        break
            except Exception as e:
                print(f"[HTTP {direction}] Forward error: {e}")
            finally:
                shutdown_event.set()

        t1 = threading.Thread(target=forward_http, args=(client_conn, remote_sock, "C→S", shutdown_event))
        t2 = threading.Thread(target=forward_http, args=(remote_sock, client_conn, "S→C", shutdown_event))
